...
import { Brawler, BrawlersApiResponse } from "../domain/Brawler";
import { HttpClient } from "../../common/domain/HttpClient";

const brawlers: BrawlersApiResponse = {
  list: [
    {
      id: 16000039,
      avatarId: 28000054,
...
export class BrawlersRepository implements BrawlerRepository {
  ...
  async get(filter: string): Promise<Either<DataError, Brawler[]>> {
    return new Promise(async (resolve, _reject) => {
      try {
        const response = await this.http.get<BrawlersApiResponse>({
          url: "/brawlers",
        });
        if (filter) {
          const filteredBrawlers = response.data.list.filter(
            (braw: Brawler) => {
          ...
          resolve(Either.right(response.data.list));
        }
      } catch (error) {
